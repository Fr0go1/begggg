...
local win = Flux:Window("You Poor :(", "Cracked by frogo", Color3.fromRGB(0, 128, 0), Enum.KeyCode.RightShift)
...
serverHopTab:Toggle("Re-Execute", "Executes the script after server hop\nTurn this off if you use autoexec", getgenv().settings.reex, function(t)
    if t and not exd then
        queueonteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/Fr0go1/begggg/main/monkey'))()")
        exd = true
    end
    if settingsLock then return end
